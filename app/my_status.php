...
$stmt = $pdo->prepare("
  SELECT t.title,
         ...
         st.updated_at
    FROM tasks t
    LEFT JOIN student_tasks st ON st.task_id=t.id AND st.user_id=:u
   ORDER BY t.id
");
$stmt->execute([':u'=>$uid]);
$rows = $stmt->fetchAll();
...
<link rel="stylesheet" href="../CSS/01_main.css"><link rel="stylesheet" href="../CSS/addons_login.css">
<div class="container">
  <div class="hero"><h1>Mein Status</h1><hr></div>
  <table class="table">
    <thead><tr><th>Aufgabe</th><th>Status</th><th>Zuletzt geändert</th></tr></thead>
    <tbody>
